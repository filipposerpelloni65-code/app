...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = $db->prepare("UPDATE dealer_locations SET name=?, code=?, address=?, city=?, phone=?, email=?, contact_person=?, active=?, notes=? WHERE id=?");
        $stmt->execute([$name, $code, $address, $city, $phone, $email, $contact_person, $active, $notes, $id]);

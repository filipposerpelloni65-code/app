...
define('PAGE_TITLE', 'Nuovo Concessionario');
define('BREADCRUMB', ['Dashboard' => APP_URL.'/dashboard.php', 'Concessionari' => APP_URL.'/modules/dealers/index.php', 'Nuovo' => '']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            $stmt = $db->prepare("INSERT INTO dealers (name, code, email, phone, address, city, region, active, notes) VALUES (?,?,?,?,?,?,?,?,?)");
            $stmt->execute([$name, $code, $email, $phone, $address, $city, $region, $active, $notes]);
            $newId = $db->lastInsertId();
            logActivity($user['id'], 'create', 'dealer', $newId, "Creato concessionario: $name");
            header('Location: ' . APP_URL . '/modules/dealers/view.php?id=' . $newId . '&created=1');
            exit;
        } catch (Exception $e) {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="bi bi-plus-circle me-2 text-primary"></i>Nuovo Concessionario</h4>
    <a href="<?= APP_URL ?>/modules/dealers/index.php" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i>Torna alla lista</a>
</div>
...
<form method="post">
    ...
    <div class="row g-3 mb-3">
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="code" class="form-control text-uppercase" required value="<?= h($_POST['code'] ?? '') ?>" placeholder="es. DLR-001">
        </div>
    </div>
    <div class="row g-3 mb-3">
        <div class="col-md-6">
    ...
    <div class="d-flex gap-2">
        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i>Crea Concessionario</button>
        <a href="<?= APP_URL ?>/modules/dealers/index.php" class="btn btn-light">Annulla</a>
    </div>
</form>
</div>
</div>

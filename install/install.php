...
try {
    ...
    $tables = [
        ...
        "CREATE TABLE IF NOT EXISTS spedizioni (id INT AUTO_INCREMENT PRIMARY KEY, ticket_id INT NULL, request_id INT NULL, dealer_id INT NULL, location_id INT NULL, corriere VARCHAR(100), numero_tracking VARCHAR(100), status ENUM('da_spedire','spedita','consegnata','annullata') NOT NULL DEFAULT 'da_spedire', mittente VARCHAR(255), destinatario VARCHAR(255), indirizzo_spedizione TEXT, note TEXT, data_spedizione DATE NULL, data_consegna_prevista DATE NULL, data_consegna DATE NULL, created_by INT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY (ticket_id) REFERENCES tickets(id), FOREIGN KEY (request_id) REFERENCES spare_parts_requests(id), FOREIGN KEY (created_by) REFERENCES users(id))"
    ];
    ...
    $pdo->exec("INSERT IGNORE INTO modules (name, slug, description, version, enabled, icon, sort_order) VALUES
        ...
        ('Spedizioni','spedizioni','Gestione spedizioni spare parts e accessori','1.0.0',1,'bi-truck',6),

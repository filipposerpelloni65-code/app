...
try {
    ...
    $tables = [
        ...
        "CREATE TABLE IF NOT EXISTS tickets (id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(255) NOT NULL, description TEXT NOT NULL, status ENUM('open','in_progress','waiting','resolved','closed') NOT NULL DEFAULT 'open', priority ENUM('low','medium','high','urgent') NOT NULL DEFAULT 'medium', tipo_intervento ENUM('onsite','onsite_sostituzione','solo_spedizione') NOT NULL DEFAULT 'onsite', category_id INT, created_by INT NOT NULL, assigned_to INT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, closed_at TIMESTAMP NULL, FOREIGN KEY (created_by) REFERENCES users(id), FOREIGN KEY (assigned_to) REFERENCES users(id), FOREIGN KEY (category_id) REFERENCES ticket_categories(id))",
        ...
        "CREATE TABLE IF NOT EXISTS dealers (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) NOT NULL, code VARCHAR(50) NOT NULL UNIQUE, email VARCHAR(100), phone VARCHAR(50), address VARCHAR(255), city VARCHAR(100), zip VARCHAR(10) NULL, province VARCHAR(2) NULL, region VARCHAR(100), active TINYINT(1) NOT NULL DEFAULT 1, notes TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)",
        "CREATE TABLE IF NOT EXISTS dealer_locations (id INT AUTO_INCREMENT PRIMARY KEY, dealer_id INT NOT NULL, name VARCHAR(255) NOT NULL, code VARCHAR(50), address VARCHAR(255), city VARCHAR(100), zip VARCHAR(10) NULL, province VARCHAR(2) NULL, phone VARCHAR(50), email VARCHAR(100), contact_person VARCHAR(100), active TINYINT(1) NOT NULL DEFAULT 1, notes TEXT, codice_aams VARCHAR(50) NULL, id_punto_vendita VARCHAR(50) NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (dealer_id) REFERENCES dealers(id) ON DELETE CASCADE)",
        ...
        "CREATE TABLE IF NOT EXISTS spedizioni (id INT AUTO_INCREMENT PRIMARY KEY, tracking_number VARCHAR(100) NULL, corriere VARCHAR(100) NULL, status ENUM('bozza','da_spedire','spedita','consegnata','annullata') NOT NULL DEFAULT 'bozza', ticket_id INT NULL, spare_parts_request_id INT NULL, dealer_id INT NULL, location_id INT NULL, note TEXT NULL, data_spedizione DATE NULL, data_consegna_prevista DATE NULL, brt_consignee_json TEXT NULL, num_colli INT NOT NULL DEFAULT 1, peso_kg DECIMAL(8,2) NOT NULL DEFAULT 1.00, brt_labels_json LONGTEXT NULL, brt_parcel_id VARCHAR(18) NULL, brt_numeric_ref INT NULL, brt_label_stream LONGTEXT NULL, transmitted_at TIMESTAMP NULL, created_by INT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY (ticket_id) REFERENCES tickets(id) ON DELETE SET NULL, FOREIGN KEY (spare_parts_request_id) REFERENCES spare_parts_requests(id) ON DELETE SET NULL, FOREIGN KEY (dealer_id) REFERENCES dealers(id) ON DELETE SET NULL, FOREIGN KEY (created_by) REFERENCES users(id))",
        ...
        "CREATE TABLE IF NOT EXISTS borderi (id INT AUTO_INCREMENT PRIMARY KEY, data_bordero DATE NOT NULL, created_by INT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, note TEXT NULL, spedizioni_ids TEXT NOT NULL, shipped_count INT NOT NULL DEFAULT 0, FOREIGN KEY (created_by) REFERENCES users(id))"
    ];
    ...
    foreach ([
        ...
        "ALTER TABLE tickets ADD COLUMN tipo_intervento ENUM('onsite','onsite_sostituzione','solo_spedizione') NOT NULL DEFAULT 'onsite'",
        ...
        "ALTER TABLE dealer_locations ADD COLUMN codice_aams VARCHAR(50) NULL",
        "ALTER TABLE dealer_locations ADD COLUMN id_punto_vendita VARCHAR(50) NULL",
        // ZIP / province for dealers and dealer_locations (for BRT auto-fill)
        "ALTER TABLE dealers ADD COLUMN zip VARCHAR(10) NULL",
        "ALTER TABLE dealers ADD COLUMN province VARCHAR(2) NULL",
        "ALTER TABLE dealer_locations ADD COLUMN zip VARCHAR(10) NULL",
        "ALTER TABLE dealer_locations ADD COLUMN province VARCHAR(2) NULL",
        // Bordero/BRT workflow columns (safe for existing installs)
        "ALTER TABLE spedizioni ADD COLUMN brt_consignee_json TEXT NULL",
        "ALTER TABLE spedizioni ADD COLUMN num_colli INT NOT NULL DEFAULT 1",
        "ALTER TABLE spedizioni ADD COLUMN peso_kg DECIMAL(8,2) NOT NULL DEFAULT 1.00",
        "ALTER TABLE spedizioni ADD COLUMN brt_labels_json LONGTEXT NULL",
        "ALTER TABLE spedizioni ADD COLUMN brt_parcel_id VARCHAR(18) NULL",
    ...
    // Update spedizioni status ENUM to include 'bozza' (safe for existing installs)
    try {
        $pdo->exec("ALTER TABLE spedizioni MODIFY COLUMN status ENUM('bozza','da_spedire','spedita','consegnata','annullata') NOT NULL DEFAULT 'bozza'");

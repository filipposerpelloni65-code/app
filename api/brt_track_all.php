...
foreach ($rows as $row) {
    ...
    if ($isDelivered) {
        ...
        $stats['updated']++;
    } elseif ($isInTransit && $row['status'] === 'da_spedire') {
        // Package picked up — advance to spedita
        $db->prepare("UPDATE spedizioni SET status='spedita', updated_at=NOW() WHERE id=?")->execute([$row['id']]);
        logActivity($userId, 'auto_update', 'spedizione', (int)$row['id'], 'Status aggiornato a spedita via brt_track_all (collo ritirato)');
        $stats['updated']++;
    } else {
        $stats['unchanged']++;
    }
